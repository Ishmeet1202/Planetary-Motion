...
def func(initial,t):
    ...
    dvxdt = -(GM * (x/((x**2 + y**2)**(3/2))))
    dvydt = -(GM * (y/((x**2 + y**2)**(3/2))))
    
    return [vx,vy,dvxdt,dvydt]

def leap_frog(x,y,vx,vy,acc_x,acc_y,r,t,h,GM):
    list_1 = []
    vx_cord = vx[0] ; vy_cord = vy[0] ; time = t[0] ; x_cord = x[0] ; y_cord = y[0]
    while y_cord>=0:
...
def angular_momentum(mu,vx,vy,r):
    L_list = []
...
def third_law(t,GM,semimajor):
    print("\nKEPLER'S THIRD LAW:")
    print("\nValue of orbital period is:",t[-1],"seconds or",t[-1]/(24*60*60),"days")
    lhs = (t[-1]**2/semimajor**3)
    rhs = (4*np.pi*np.pi)/GM
    print("\nValue of LHS:",lhs)
    print("\nValue of RHS:",rhs)
    
def comparision(sol_an,x,y,vx,vy):
    ...
    print("\nAverage of absolute error in x coordinate (b/w odeint and leap frog) is:",np.mean(error_x),"meters")
    print("\nAverage of absolute error in y coordinate (b/w odeint and leap frog) is:",np.mean(error_y),"meters")
    print("\nAverage of absolute error in velocity x coordinate (b/w odeint and leap frog) is:",np.mean(error_vx),"meter per second")
    print("\nAverage of absolute error in velocity y coordinate (b/w odeint and leap frog) is:",np.mean(error_vy),"meter per second")

def text_files(x,y,vx,vy,acc_x,acc_y,r,t,sol_an,t1):
...
if __name__ == "__main__":
    ...
    r = []
    t = [time]
    initial = [x_initial,y_initial,vx_initial,vy_initial] # FOR ODEINT FUNCTION

    r.append(np.sqrt(x[0]**2 + y[0]**2))
